...
public class DeleteADController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = ERROR;
        try {
            String roomID = request.getParameter("txtDelete");
            RoomDAO room = new RoomDAO();
            HttpSession session = request.getSession();
            List<RoomDTO> listAD = null;
            if (room.DeleteRoom(roomID)) {
                request.setAttribute("DELETEAD", roomID + " Deleted");
                listAD = room.getListRoomAD();
                session.setAttribute("LISTAD", listAD);
                url = SUCCESS;
            }
        } catch (Exception e) {
            Logger.getLogger(DeleteADController.class.getName()).error(e.getMessage());
        } finally {
